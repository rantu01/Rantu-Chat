...
export default function Settings({ user, onSettingsUpdated, token }: SettingsProps) {
  ...
  const [delayValue, setDelayValue] = useState(Math.max(2, user.autoReplyDelaySeconds));
    ...
    >
      ...
      <div className="mb-6 rounded-2xl border border-zinc-800 bg-zinc-950/60 p-4">
        ...
        <div className="flex gap-2">
          <input
            id="settings-delay-value-input"
            type="number"
            min={delayUnit === "seconds" ? 2 : 1}
            step={1}
            ...
            value={delayValue}
            onChange={(e) => setDelayValue(Math.max(delayUnit === "seconds" ? 2 : 1, Number(e.target.value) || 0))}
          />
          <select
            id="settings-delay-unit-select"
            className="flex-1 rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-2.5 text-xs text-zinc-100 outline-none focus:border-zinc-700 transition-all font-mono"
            value={delayUnit}
